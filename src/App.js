...
function App() {
  ...
  const SHEETDB_URL = 'https://sheetdb.io/api/v1/flzw5cts0b4na';
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      fetchFutureMeals(user.memberNumber);
    }
  }, []);
  ...
  const fetchFutureMeals = async (memberNumber) => {
    const today = new Date().toISOString().split('T')[0];
    const res = await fetch(`${SHEETDB_URL}/search?memberNumber=${memberNumber}`);
    const data = await res.json();
    const upcoming = data.filter(row => row.date >= today);
    setFutureMeals(upcoming);
  };
